...
import { getCheatsheets } from "@/utils/mdx";
...
const Cheatsheets = () => {
  ...
  const filtered = useMemo(() => {
    return cheatsheets.filter((sheet) => {
      ...
      const excerpt = sheet?.excerpt?.toLowerCase() || '';
    ...
    <div className="min-h-screen bg-white text-gray-900 dark:bg-gray-900 dark:text-gray-100 transition-colors duration-300">
      ...
      <main className="max-w-2xl mx-auto px-6 py-12">
        <header className="mb-10">
          <h1 className="text-3xl font-serif font-light text-amber-500 dark:text-blue-200 mb-2">Cheatsheets</h1>
          <p className="text-lg text-gray-600 dark:text-gray-300 font-light">
            Handy coding references for quick lookup.
          </p>
          <div className="mt-6">
            <input
              type="text"
        ...
        {isLoading ? (
          <div className="text-center text-gray-500">Loading cheatsheets...</div>
        ) : (
          <>
            <section className="space-y-12">
              {paginated.map((sheet) => (
                ...
                >
                  <Link to={`/cheatsheets/${sheet.slug}`} className="block group">
                    ...
                    <div className="flex items-center text-sm space-x-4 text-gray-500 dark:text-gray-400 mb-2">
                      ...
                      <span>•</span>
                      <span>{sheet.readTime || '2 min read'}</span>
                    </div>
            ...
            {totalPages > 1 && (
              <div className="flex justify-center items-center gap-2 mt-10 flex-wrap">
                <button
                  onClick={handlePrev}
                  disabled={page === 1}
                  className="px-3 py-1 rounded border text-amber-600 dark:text-blue-200 text-sm bg-white dark:bg-gray-800 border-gray-300 dark:border-gray-600 disabled:opacity-40"
                >
                  Previous
                </button>
